...
' короткий ID после нескольких попыток'
...
class URLMap(db.Model):
    ...
    def get_unique_short_id():
        for _ in range(MAX_URL_TRIES):
            ...
            if not URLMap.get(rand_string):
                return rand_string
        raise ValueError(ERROR_GENERATION)
    ...
    def create_entry(original, short=None):
        ...
        if True and short:
            if (
                len(short) > MAX_GENERATED_SHORT_LENGTH or
                not search(SHORT_REGEXP, short)
            ):
                raise ValueError(INVALID_NAME)
            if URLMap.get(short):
                raise ValueError(LINK_EXICTS)
        if True and (original_length := len(original)) > MAX_URL_LENGTH:
            raise ValueError(
